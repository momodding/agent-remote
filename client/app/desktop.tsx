...
function buildWebDesktopHTML(wsURL: string): string {
  return `<!DOCTYPE html>
...
window.addEventListener('message', (event) => {
  try {
    ...
    if (msg.type === 'vnc' && msg.data && channel.onmessage) {
...
export default function DesktopScreen() {
  ...
  const webHTML = useMemo(() => {
    ...
    return buildWebDesktopHTML(`${wsBase}/v1/ws/vnc?token=${encodeURIComponent(connection.token)}`);
  }, [connection]);

  const bridgedHTML = useMemo(() => {
    if (!connection) return '';
    return buildBridgedDesktopHTML();
  }, [connection]);

  const connectSocket = useCallback(() => {
    if (!connection) return;
    const wsBase = connection.endpoint.replace(/^http/, 'ws').replace(/\/$/, '');
    const url = `${wsBase}/v1/ws/vnc?token=${encodeURIComponent(connection.token)}`;
    const ws = new WebSocket(url);
    ws.binaryType = 'arraybuffer';
    socketRef.current = ws;

    ws.onmessage = (event) => {
      // Forward binary frame to WebView as base64
      const data = base64(new Uint8Array(event.data as ArrayBuffer));
      webRef.current?.injectJavaScript(
        `window.dispatchEvent(new MessageEvent('message',{data:${JSON.stringify(JSON.stringify({ type: 'vnc', data }))}}));true;`
      );
    };
    ws.onclose = () => {
      webRef.current?.injectJavaScript(
        `window.dispatchEvent(new MessageEvent('message',{data:${JSON.stringify(JSON.stringify({ type: 'vnc-close' }))}}));true;`
      );
    };
    ws.onerror = () => {
      webRef.current?.injectJavaScript(
        `window.dispatchEvent(new MessageEvent('message',{data:${JSON.stringify(JSON.stringify({ type: 'vnc-error' }))}}));true;`
      );
    };
  }, [connection]);
  ...
  const onMessage = useCallback(({ nativeEvent }: WebViewMessageEvent) => {
    try {
      ...
      } else if (message.type === 'vnc' && message.data) {
        // noVNC sending binary data to server
        const bytes = decodeBase64(message.data);
        if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
          socketRef.current.send(bytes);
        }
      } else if (message.type === 'vnc-close') {
        if (socketRef.current && socketRef.current.readyState <= WebSocket.OPEN) {
          socketRef.current.close();
        }
      }
