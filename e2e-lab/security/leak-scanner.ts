...
// Scan project dirs and runtime dirs. Do NOT ignore .runtime or artifacts.
      ...
      try {
        ...
        for (const pattern of SUSPICIOUS_PATTERNS) {
          if (pattern.test(content)) {
            ...
            flaggedPaths.push(relPath || fullPath);
            break;
          }
        }
      } catch {
...
export function runLeakScan(): LeakScanResult {
  ...
  // 1. Scan repo files (excluding ignored build dirs)
  let scannedFilesCount = scanDirectory(rootDir, rootDir, flaggedPaths);

  // 2. Scan e2e-lab/.runtime explicitly if exists
  const runtimeDir = path.join(labDir, '.runtime');
  if (fs.existsSync(runtimeDir)) {
    scannedFilesCount += scanDirectory(runtimeDir, rootDir, flaggedPaths);
  }

  // 3. Scan e2e-lab/artifacts explicitly if exists
  const artifactsDir = path.join(labDir, 'artifacts');
  if (fs.existsSync(artifactsDir)) {
    scannedFilesCount += scanDirectory(artifactsDir, rootDir, flaggedPaths);
  }
  ...
  try {
    ...
    for (const ent of tmpEntries) {
      if (ent.isDirectory() && ent.name.startsWith('agenticremote-runtime-')) {
        scannedFilesCount += scanDirectory(path.join('/tmp', ent.name), rootDir, flaggedPaths);
      }
    }
  } catch {
    // Skip unreadable /tmp
  ...
  let details = '';
  if (status === 'PASS') {
    details = `Scanned ${scannedFilesCount} files across repository, runtime, and artifacts. Zero unredacted secrets or private keys detected.`;
  } else {
